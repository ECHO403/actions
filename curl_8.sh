...
build_x86() {
  ...
  ./configure --host x86 --with-pic --with-openssl --prefix=/opt/curl/${arch}
  make && make install 
  mkdir -p /tmp/curl/${arch} && mv /opt/curl/${arch}/* /tmp/curl/${arch}/
  make clean
}
...
build_arm() {
  ...
  ./configure --host arm-linux-androideabi --with-pic --with-openssl --prefix=/opt/curl/${arch}
  make && make install 
  mkdir -p /tmp/curl/${arch} && mv /opt/curl/${arch}/* /tmp/curl/${arch}/
  make clean
}
...
build_arm64() {
  ...
  ./configure --host aarch64-linux-android --with-pic --with-openssl --prefix=/opt/curl/${arch}
  make && make install 
  mkdir -p /tmp/curl/${arch} && mv /opt/curl/${arch}/* /tmp/curl/${arch}/
  make clean
}
...
build_x86_64() {
  ...
  ./configure --host x86_64 --with-pic --with-openssl --prefix=/opt/curl/${arch}
  make && make install 
  mkdir -p /tmp/curl/${arch} && mv /opt/curl/${arch}/* /tmp/curl/${arch}/
  make clean
}
